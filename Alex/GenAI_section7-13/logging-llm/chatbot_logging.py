...
def initialize_client(use_ollama: bool = True) -> OpenAI:
    ...
    if use_ollama:
        return OpenAI(base_url="http://localhost:11434/v1", api_key="ollama") #If true use local LLM: Ollama
    return OpenAI() #If false, then use OpenAI ChatGPT
...
class ChatBot:
    ...
    def chat(self, user_input: str) -> str:
        try:
            ...
            # assistant_response = response.choices[0].message.content #If you dont stream it then just need this
            ...
            self.messages.append({"role": "assistant_response", "content": assistant_response})
