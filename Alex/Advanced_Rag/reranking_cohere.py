...
class CohereReranker:
    ...
    def rerank(self, query: str, documents: List[Document], top_k: int=3) -> List[Dict]:
        try: 
            docs = [str(doc.page_content) for doc in documents]
            
            # Perform reranking:
            response = co.rerank(
                model="rereank-v3.5", query=query, documents= docs, top_n= top_k
            )
            reranked_results = []
            ...
            # Fallback return original documents with default scoring. This is more or less made up:
            return [
